  ...
  $(document).ready(function() {
    ...
    $('.btn_more_park').on('click', function() {
      event.preventDefault();
      $('html,body').animate({
        scrollTop: $('#bus-slider_wrap').offset().top + "px"
      }, {
        duration: 1E3
      });
    });
    ...
    $('.btn-license').on('click', function() {
      event.preventDefault();
      $('html,body').animate({
        scrollTop: $('.license_block').offset().top + "px"
      }, {
        duration: 1E3
      });
    });
      ...
      forEach(parent_block.getElementsByClassName('bus-slider_main_content'), function(index, value) {
        ...
        if (value.dataset.id == id_active) {
          ...
          if (value.dataset.id == '#three') {
            ...
            //   offset: 100 // Loads images 100px before they're visible
            // });
            var slisk_bus = $(value.querySelectorAll('.gallery__bus_wrap')[0]).slick({
              // dots: true,
              // infinite: true,
              speed: 300,
              slidesToShow: 1,
            ...
            // });
            //
            // slisk_bus.on('afterChange', bLazy.revalidate);



            $('.gallery__bus_control-right').on('click', function() {
              slisk_bus.slick('slickNext');
            });
            $('.gallery__bus_control-left').on('click', function() {
              slisk_bus.slick('slickPrev');
            });
          }
