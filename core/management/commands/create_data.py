...
from djfilter.urls import Journal, Author2, Category
import random
import datetime 
...
class Command(BaseCommand):
  def add_arguments(self, parser):
    parser.add_argument(
      'file_name', 
      type=str, 
      help='The .txt file, that contains the journal titles'
    )
  def handle(self, *args, **kwargs):
    ...
    with open(f'{file_name}.txt') as file:
      for row in file:
        ...
        author = Author2.objects.get_or_create(
          name = author_name
        )
        journal = Journal(
          title=title,
          author=Author2.objects.get(name = author_name),
